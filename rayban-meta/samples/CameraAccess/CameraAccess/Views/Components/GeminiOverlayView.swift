...
struct GeminiStatusBar: View {
  ...
  var body: some View {
    HStack(spacing: 8) {
      ...
      // MCP connection pill
      StatusPill(color: mcpStatusColor, text: mcpStatusText)
    }
  }
  ...
  private var mcpStatusColor: Color {
    switch geminiVM.mcpConnectionState {
  ...
  private var mcpStatusText: String {
    switch geminiVM.mcpConnectionState {
    case .connected(let name): return "MCP: \(name)"
    case .checking: return "MCP..."
    case .unreachable: return "MCP Off"
    case .notConfigured: return "No MCP"
    }
  }
}
